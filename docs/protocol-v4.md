# MusicBee Remote Protocol - V4 / V5 (Legacy)

This document describes the **legacy V4 / V5 protocol**: the CRLF-delimited
`{"context":..,"data":..}` protocol that shipping Android (1.1.0-1.6.1) and iOS clients speak.

> **V6 lives in a separate document.** V6 is the clean-slate protocol (JSON envelope,
> newline framing, string enums, typed fields). See [protocol-v6.md](protocol-v6.md). V4/V5
> and V6 coexist on the same TCP port; the server routes by the shape of the first frame.
> V4/V5 are a **frozen legacy target** - their quirks are preserved byte-for-byte for
> compatibility and are not extended; new protocol work happens in V6.

It documents the **maintained protocol-4 surface** plus the single V5 alias: the commands that
shipping Android and iOS clients actually use and that we intend to keep supporting. Pre-V4
