...
mod ffi_boundary_smoke {
    ...
    extern "C" fn fake_query(
        ...
        // SAFETY: the caller is the test below, which passes the pointer and
        // length of a live msgpack buffer that outlives this call.
        ...
        // SAFETY: both out-params are non-null locals owned by the caller below,
        // and the buffer they receive is leaked, so it outlives the write.
        unsafe {
            *out_buf = ptr;
            *out_len = len;
        }
