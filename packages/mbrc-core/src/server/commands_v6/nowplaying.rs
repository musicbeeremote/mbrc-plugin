//! V6 now-playing STATE domain: the current track and its facets.
//!
//! A composite `now_playing_state` read plus extended details, position,
//! structured lyrics (#113), and the granular writes (seek / rating / last.fm /
//! tag edit).
//!
//! The now-playing LIST (`now_playing_list` plus play / remove / move / search /
//! queue) lives in [`nowplaying_list`](super::nowplaying_list).
...
pub fn dispatch(
    ...
    Some(match op {
        "now_playing_state" => state(data, p, now_playing, cover_store),
...
///
/// The track's `duration_ms` is the player's exact one, not the second-granular
/// value parsed from the tag (#112): two durations in one response is a seek bar
/// that drifts. `list_order` is opt-in, since only a client windowing the queue
/// needs it.
fn state(
    data: &Value,
    p: &dyn Providers,
    ...
    if pos.total > 0
        && let Some(obj) = track.as_object_mut()
    {
        obj.insert("duration_ms".to_owned(), json!(pos.total));
    }
    let list_order = if opt_bool(data, "include_list_order")?.unwrap_or(false) {
        current_list_order(p)?
    } else {
        None
    };
    Ok(json!({
...
/// The playing track's absolute index in the now-playing list, or `None` when
/// nothing is playing.
///
/// Read off the forward play-order walk, whose first item is by definition the
/// current track and whose `position` is its storage index - one provider call,
/// and no new FFI surface.
fn current_list_order(p: &dyn Providers) -> Result<Option<i64>, V6Error> {
    Ok(p.now_playing_list_ordered(0, 1)
        .map_err(internal)?
        .data
        .first()
        .map(|npt| npt.position as i64))
}
...
/// Parses an LRC time tag `mm:ss`, `mm:ss.xx`, or `mm:ss.xxx` to milliseconds.
///
/// The arithmetic is checked, like `parse_duration_ms`: a pathological timestamp
/// such as `[9999999999999:00]` overflows i64, and a wrapped negative `at_ms`
/// would reach the client as a real position.
...
mod tests {
    ...
    /// The tag string is second-granular, the player's own duration is not, and
    /// a response carrying both must not disagree with itself (#112).
    #[test]
    fn the_playing_track_takes_the_exact_duration_not_the_parsed_one() {
        let m = MockProviders {
            track_info: TrackInfo {
                path: "a.mp3".into(),
                ..Default::default()
            ...
            tracks_detailed: vec![TrackTags {
                src: "a.mp3".into(),
                duration: "3:00".into(), // parses to 180_000
                ..Default::default()
            }],
            position: PlaybackPositionResponse {
                current: 1_000,
        ...
        let out = run("now_playing_state", json!({}), &m).unwrap();
        assert_eq!(out["track"]["duration_ms"], 180_345);
        assert_eq!(out["duration_ms"], out["track"]["duration_ms"]);
    }

    /// #118 §7 windows the queue around the current track from the client side,
    /// which needs the current track's absolute index to window *from*.
    #[test]
    fn list_order_is_returned_only_when_asked_for() {
        let m = MockProviders {
            track_info: TrackInfo {
                path: "a.mp3".into(),
                ..Default::default()
            },
            now_playing_list_ordered: crate::protocol::messages::Page {
                total: 1,
                offset: 0,
                limit: 1,
                data: vec![crate::protocol::messages::NowPlayingListTrack {
                    path: "a.mp3".into(),
                    position: 17, // the storage index of the playing track
                    ..Default::default()
                }],
            },
            ..Default::default()
        ...
        let out = run(
            "now_playing_state",
            json!({ "include_list_order": true }),
            &m,
        )
        .unwrap();
        assert_eq!(out["list_order"], 17);
    }

    #[test]
    fn list_order_is_null_when_the_queue_is_empty() {
