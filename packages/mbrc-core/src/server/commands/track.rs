...
//! rating, Last.fm rating, and tag changes.
//!
...
/// Serializes a DTO onto a context as its reply frame.
...
/// Seeks (int ms) or queries (null/`"status"`); replies with the current
/// `{current,total}`.
///
...
/// V5 iOS `nowplayingcurrentposition` alias: report the current
/// `{current,total}` on the `nowplayingposition` context.
///
/// Query-only - unlike `position` it never seeks, matching the C#
/// `RequestCurrentPosition`, which ignores the payload and always reports.
///
/// # Errors
/// The provider call failed.
...
/// Now-playing cover on request.
///
/// The C# host hands over the raw MusicBee artwork; the core owns sizing and
/// resizes it to the 600px now-playing default (matching the shipped plugin).
/// On any resize failure we log and send the original bytes rather than
/// dropping the cover.
///
/// # Errors
/// The provider call failed.
...
/// Sets (`"0"`-`"5"`, or `""` to clear) or queries (null).
///
/// A set reads back fresh from the provider; a query serves from the cache.
/// Android sends the rating as a string and iOS as a bare number, and both must
/// count as a set: treating only strings that way sent an iOS rating down the
/// query branch, where it changed nothing and returned the stale value.
///
/// # Errors
/// The provider call failed.
...
/// Changes a metadata tag on the current track; reply the updated details on the
