//! V6 wire protocol primitives.
//!
//! The strict JSON envelope, defined string enums, newline framing, and the
//! client/server frame builders. Shared by the core server, the `mbrc` CLI and
//! the api-debugger backend, so the envelope has a single definition.
//!
...
/// The closed set of framing and handshake error codes.
///
/// An op that needs a code of its own adds it here; the set is closed so a
/// client can match on it exhaustively.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ErrorCode {
...
/// Parses and structurally validates an inbound request line.
///
/// Enforces the envelope shape (a JSON object with a numeric `id`,
/// `kind == "request"` and a string `op`); `data` defaults to `null` when
/// absent. Unknown extra envelope keys are ignored - only structure is
/// validated (#118 §2.1).
